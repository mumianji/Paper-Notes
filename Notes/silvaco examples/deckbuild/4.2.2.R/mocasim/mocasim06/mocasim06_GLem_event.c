...
void scattering_event(double* kx, double* ky, double* kz, long* seed) {
  ...
  mag_kG=sqrt((*kx)*(*kx) + (*ky)*(*ky) + (*kz)*(*kz));
  EG=(-1.0 + sqrt(1.0 + 1.4827e-14*mag_kG*mag_kG)) / 1.226;

  /* calculate the energy and wavevector after being scattered */
  EL=EG -0.33 -0.02788;
  if (EL<=0.0) return; /* assume no scattering occurs */

  mag_kL=2.4139e7*sqrt(EL*(1.0 + 0.463*EL));
  ...
  (*kx)=mag_kL*sin(phi)*cos(theta);
  (*ky)=mag_kL*sin(phi)*sin(theta);
  (*kz)=mag_kL*cos(phi);
}
